...
// jiyu yan ID: 1851015 for CS402 HPC assignment2
...
//int *ileft, *iright;    change pointer to integer type for convenience
int ileft, iright;           /* Array bounds for each processor */
int each_part;              // size of each part, from ileft to iright
...
int main(int argc, char *argv[])
{
    ...
    // intend to print out time cost, not conveient and accurate as gprof, bring unnecessary overhead, so comment out
    // double mainloopTime = 0.0;
    // double possionTime = 0.0;
    // double count = 0.0;
    // double tempTime;
    // if (proc == 0){
    //   totalTime = MPI_Wtime();
    //   mainloopTime = MPI_Wtime();
    // }
    ...
    if (imax % nprocs != 0) {
  // Although I tried to divide the data when it couldn't be divided by the number of prcessors in integer,
  // the bin files differ(something goes wrong). So i need to check it at first to guarantee correct results
      if (proc == 0){
        printf("Imax can't be divided by %d.\n", nprocs);
        MPI_Abort( MPI_COMM_WORLD, 1 );
        return -1;
      }
    }
    // if using imax/nprocs directly rather than each_part, the bin files we got differ.
    each_part = imax / nprocs;
    // the left bound of each processor
    ileft = proc * each_part + 1;
    // the right bound of each processor, make sure the last one is right, which couldn't bigger than imax.
    iright = min(imax, each_part * (proc + 1));
